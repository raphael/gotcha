...
func init() {
  ...
  if confFile, err := filepath.Abs(confFile); err != nil {
    ...
    log.Printf("Cannot load configuration file '%s', using default settings", confFile)
  } else {
  ...
  StartSession(globalSettings["mongoHost"], globalSettings["mongoUser"],
...
func main() {
	...
	m.Post("/projects/:projectName/queues", http.HandlerFunc(createQueue))
...
func createProject(w http.ResponseWriter, req *http.Request) {
  ...
  if p, err := NewProject(name); err != nil {
...
func showProject(w http.ResponseWriter, req *http.Request) {
  name := req.URL.Query().Get(":projectName")
  if p, err := LoadProject(name); err != nil {
...
func createQueue(w http.ResponseWriter, req *http.Request) {
  if p, err := findProject(w, req); err != nil {
    ...
    if q, err := NewQueue(name, p); err != nil {
      http.Error(w, fmt.Sprintf("Failed to create queue: %s", err), 422)
    } else if info, err := q.Info(); err != nil {
...
func addMessages(w http.ResponseWriter, req *http.Request) {
  ...
  for _, m := range messages {
    ...
    expiresIn, err := extractDuration(m["expires_in"], MinMessageExpiry, MaxMessageExpiry, DefaultMessageExpiry)
    if err != nil {
      http.Error(w, fmt.Sprintf("Badly formed request: %s (expires_in)", err), 400)
      return
    }
    internalMsgs = append(internalMsgs, Message{Body: body, QueueID: q.ID, ProjectID: q.ProjectID,
                                                ExpiresAt: now.Add(expiresIn), CreatedAt: now})
  }
  err = SaveMessages(&internalMsgs)
  if err != nil {
    http.Error(w, fmt.Sprintf("Failed to enqueue messages: %s", err), 422)
    return
  }
  ids := make([]string, len(internalMsgs))
...
func getMessages(w http.ResponseWriter, req *http.Request) {
  ...
  if err != nil {
    http.Error(w, fmt.Sprintf("Invalid timeout value '%s' (must be an integer <= %s >= %s)", timeout, MaxMessageTimeout, MinMessageTimeout), 400)
    return
  }
...
func findProject(w http.ResponseWriter, req *http.Request) (*Project, error) {
  name := req.URL.Query().Get(":projectName")
...
func findQueue(w http.ResponseWriter, req *http.Request) (*Queue, error) {
